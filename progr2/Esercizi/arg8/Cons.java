...
public class Cons extends List {
    ...
    public int sum() { return elem + next.sum(); }
    ...
    public List filter_le(int x) {
        if(elem <= x) return new Cons(elem, next.filter_le(x));
        return new Nil();
    }

    // Ritorna una nuova sotto-lista che contiene tutti gli elementi di maggiori a @x
    @Override
    public List filter_gt(int x) {
        if(elem > x) return new Cons(elem, next.filter_gt(x));
    ...
    public List intersect(List l) {
        ...
        if(l.contains(elem)) return new Cons(elem, next.intersect(l));
        else return next.intersect(l);
    }
