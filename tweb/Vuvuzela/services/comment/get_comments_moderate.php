...
const GET_MODERATE_QUERY = 'SELECT user.username AS author, DATE_FORMAT(date, \'%M %d, %Y\') AS date, text, comment.id AS id
                            FROM comment
                                JOIN user ON comment.author_id = user.id
                            WHERE comment.id < :first AND
                                  comment.verified = 0
...
if(isset($_REQUEST['first']) &&
    ...
    $statement = $connection->prepare(GET_MODERATE_QUERY);
